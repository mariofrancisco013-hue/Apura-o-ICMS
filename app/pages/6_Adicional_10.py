...
with st.expander("Cadastro de clientes (aba FILTRO) — cadastro global"):
    st.caption(
        "Código do cliente (Winthor) → classificação. Só clientes classificados **Sim** têm suas NFs "
        "somadas em VENDAS. Cliente não cadastrado aqui é tratado como **Não** (não conta), igual o "
        "comportamento da planilha original."
    )
    clientes_atuais = carregar_clientes_filtro(session)
    st.caption(f"{len(clientes_atuais)} cliente(s) cadastrado(s) atualmente.")
    busca = st.text_input("Buscar por código ou nome", key="busca_cliente_adicional10")
    if busca:
        filtro_busca = (
            clientes_atuais["cliente_nome"].fillna("").str.contains(busca, case=False)
            | clientes_atuais["cod_cliente"].astype(str).str.contains(busca)
        )
        st.dataframe(clientes_atuais[filtro_busca], use_container_width=True, height=250, hide_index=True)
    else:
        st.dataframe(clientes_atuais, use_container_width=True, height=250, hide_index=True)

if cid is None:
    st.info("Importe a planilha acima para começar.")
    st.stop()
...
faturamento_input = st.number_input(
    ...
    min_value=0.0, value=float(faturamento_atual) if faturamento_atual is not None else 0.0, step=1000.0,
    format="%.2f", key="input_faturamento_adicional10",
    help="Digitado manualmente — a planilha original também não calcula esse valor por fórmula nenhuma.",
)
...
if faturamento_atual is None:
    st.warning("Faturamento ainda não foi salvo para esta competência — Base de Cálculo está usando R$ 0,00.")
...
if not detalhamento.empty:
    ...
    if not nao_classificados.empty:
        with st.expander(f"⚠️ {len(nao_classificados)} cliente(s) desta competência sem cadastro no FILTRO"):
            st.caption(
                "NFs desses clientes não estão contando em VENDAS (tratados como \"Não\") porque o código "
                "não aparece no cadastro global — classifique abaixo e salve pra passar a contar (ou "
                "confirme que devem mesmo ficar de fora)."
            )
            ...
            edit_nao_class_editada = st.data_editor(
                ...
                column_config={
                    ...
                    "calcula": st.column_config.SelectboxColumn("Classificação", options=["Sim", "Exceção"]),
                },
            )
    ...
    tabela_nfes["vl_total_fmt"] = tabela_nfes["vl_total"].apply(lambda v: formatar_moeda(v) if pd.notna(v) else "—")
    tabela_nfes["conta_fmt"] = tabela_nfes["conta"].map({True: "✅ Sim", False: "❌ Não"})
    st.dataframe(
        tabela_nfes.rename(columns={
            "nfe": "NFE", "emissao": "Emissão", "cod_cliente": "Cód. Cliente", "cliente_nome": "Cliente",
            "uf": "UF", "vl_total_fmt": "Valor Total", "conta_fmt": "Conta em VENDAS?",
